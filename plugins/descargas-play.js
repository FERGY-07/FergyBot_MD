...
await conn.sendButton(m.chat, texto1, wm, res.videos[0].thumbnail, [
  ...
  ], null, [['𝐂𝐀𝐍𝐀𝐋 𝐁𝐄𝐍𝐉𝐀 𝐆𝐎𝐑𝐃𝐎 𝐓𝐄𝐓𝐎𝐍', `${md}`]], m)
	  }
...
if (size.split('MB')[0] >= limit) return conn.reply(m.chat,`El archivo pesa mas de ${limit} MB, se canceló la Descarga.`, m, fake,).then(_ => m.react('✖️'))
    ...
    if (command == "mp3doc") {
            if (!inputs) return conn.reply(m.chat, `*\`╭━❰❰ 𝐁𝐄𝐍𝐉𝐀 𝐆𝐎𝐑𝐃𝐎 𝐓𝐄𝐓𝐎𝐍 ❱❱━╮\`*\n> 𝑽𝒖𝒆𝒍𝒗𝒆 𝒂 𝒖𝒔𝒂𝒓 𝒆𝒍 𝒄𝒐𝒎𝒂𝒏𝒅𝒐 𝒑𝒆𝒓𝒐 𝒂𝒄𝒐𝒎𝒑𝒂ñ𝒂𝒅𝒐 𝒅𝒆𝒍 𝒕𝒊𝒕𝒖𝒍𝒐 + 𝒆𝒍 𝒂𝒖𝒕𝒐𝒓.\n> 𝑬𝒋𝒆𝒎𝒑𝒍𝒐: .𝒑𝒍𝒂𝒚 𝑫𝒆𝒗𝒖𝒆𝒍𝒗𝒆𝒎𝒆 𝒂 𝒎𝒊 𝒄𝒉𝒊𝒄𝒂 - 𝑯𝒐𝒎𝒃𝒓𝒆𝒔𝑮`,  m)
    await m.react('🕓')
    let res = await yts(text)
    let vid = res.videos[0]
    let q = '128kbps'
